...
const Home = () => {
  ...
  const categories = [
    {
      name: '등',
      path: '/back',
      icon: 'M4 12h16M6 8v8M18 8v8M9 8v8M15 8v8', // 랫풀다운 기계
    },
    {
      name: '가슴',
      path: '/chest',
      icon: 'M4 12h16M6 8v8M18 8v8M9 8v8M15 8v8',
    },
    {
      name: '어깨',
      path: '/shoulder',
    ...
    { name: '심사', path: '/evalution', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
    {
      name: '루틴',
      path: '/routine',
  ...
  // 카테고리 클릭 시 해당 카테고리 페이지로 이동하는 함수
    ...
    <div className="max-w-[600px] min-h-screen ml-auto mr-auto bg-white font-bold text-3xl pb-14">
            ...
            {master.map((item, index) => (
              <div key={index} className="px-2">
                <div className="flex flex-col items-center p-4 bg-gray-100 rounded-lg">
                  ...
                  />
                  <h3 className="text-base sm:text-lg md:text-xl font-GmarketMedium">{item.name}</h3>
                  <p className="text-xs text-gray-600 sm:text-sm md:text-base">@{item.sns}</p>
                </div>
              </div>
            ))}
          </Slider>
      ...
      <div className="px-5 py-3 mt-10">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm sm:text-xl font-GmarketBold text-[#2EC4B6]">피드백 커뮤니티</h2>
          <Link
            to="/feedback"
        ...
        <div className="grid grid-cols-2 gap-4">
          {feedbackPosts.map((post) => (
            <div
              ...
              onClick={() => navigate(`/feedback/${getCategoryPath(post.category)}/${post.id}`)}
            >
